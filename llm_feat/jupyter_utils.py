...
def inject_code_to_next_cell(code: str) -> bool:
    ...
    try:
        ...
        # Method 1: Try JavaScript for immediate cell creation (works in classic Notebook)
        try:
        ...
        # Method 2: Use IPython's set_next_input (works in all environments)
        # This sets the code for the next cell that gets created
        try:
            ipython.set_next_input(code, replace=False)
            if not injected:
                print("✓ Code ready for next cell - create a new cell below and it will appear automatically")
            else:
                print("  (Also set as next input - will appear in next cell you create)")
            return True
        except Exception as e2:
            if injected:
                return True  # JavaScript worked, even if set_next_input failed
            pass
