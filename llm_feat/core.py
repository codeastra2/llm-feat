...
from .jupyter_utils import is_jupyter, inject_code_to_next_cell, get_code_string
            ...
            for col in categorical_cols:
                if col in df.columns:
                    ...
                    if unique_count <= 20:  # Show all unique values if <= 20
                        ...
                        info_lines.append(f"    Sample unique values (first 10): {sample_vals}")
                        info_lines.append(f"    ... and {unique_count - 10} more unique values")
                    # Add value counts for top categories
                    value_counts = df[col].value_counts().head(5)
                    info_lines.append(f"    Top 5 value counts:")
                    for val, count in value_counts.items():
                        info_lines.append(f"      '{val}': {count} ({count/len(df)*100:.1f}%)")
...
def _prepare_metadata_info(metadata_df: pd.DataFrame) -> str:
    ...
    required_cols = ['column_name', 'description', 'data_type', 'label_definition']
    
    # Validate metadata structure
    missing_cols = [col for col in required_cols if col not in metadata_df.columns]
    if missing_cols:
        raise ValueError(
            f"Metadata DataFrame missing required columns: {missing_cols}. "
...
def _extract_target_column(metadata_df: pd.DataFrame) -> Optional[str]:
    ...
    if 'label_definition' in metadata_df.columns:
        label_rows = metadata_df[
            metadata_df['label_definition'].notna() & 
            (metadata_df['label_definition'] != '')
        ]
...
def _get_categorical_columns(df: pd.DataFrame, metadata_df: pd.DataFrame) -> list:
    ...
    if 'data_type' in metadata_df.columns and 'column_name' in metadata_df.columns:
        cat_rows = metadata_df[
            metadata_df['data_type'].str.lower().isin(['categorical', 'category', 'cat', 'string', 'text', 'object'])
        ]
        categorical_cols = [row['column_name'] for _, row in cat_rows.iterrows() 
...
def generate_features(
    ...
    Args:
        ...
        metadata_df: Metadata DataFrame with columns: column_name, description, 
                    data_type, label_definition. Set data_type to 'categorical' 
                    for categorical columns to enable categorical feature engineering.
        mode: 'code' to return/suggest code, 'direct' to add features directly
        api_key: OpenAI API key (optional if already set via set_api_key())
        model: OpenAI model to use (default: "gpt-4o", alternatives: "gpt-4-turbo", "gpt-3.5-turbo")
        debug: If True, print the generated code before execution (useful for troubleshooting)
        
    Note:
        Generated code uses 'df' as the DataFrame variable name. 
        If your DataFrame has a different name, replace 'df' with your variable name in the generated code.
        
    Returns:
        - If mode='code': Returns code string (and injects to next cell if in Jupyter)
        - If mode='direct': Returns DataFrame with new features added
    
    Note:
        Categorical features are automatically detected from metadata_df where 
        data_type is 'categorical', 'category', 'cat', 'string', 'text', or 'object'.
        The LLM will generate appropriate encoding strategies (one-hot, target encoding, 
        frequency encoding, etc.) based on the unique value counts.
    """
    ...
    if mode == 'direct':
        ...
        has_df_assignments = (
            "df['" in generated_code or 
            'df["' in generated_code or
            "df.loc" in generated_code or
            "pd.get_dummies" in generated_code
        )
        if not has_df_assignments:
            import warnings
            warnings.warn(
                f"Generated code may not create new columns. "
                f"Code should contain patterns like df['new_col'] = ... "
                f"Generated code:\n{generated_code[:500]}"
            )
    ...
    if mode == 'code':
        ...
        try:
            ...
            # Note: The code should modify 'df' in place (e.g., df['new_col'] = ...)
            exec(generated_code, safe_globals)
            
            # After execution, get the DataFrame from safe_globals
            # This ensures we get the modified version even if code reassigned df
            ...
            if not isinstance(df_result, pd.DataFrame):
                raise RuntimeError(
                    f"After code execution, 'df' is not a DataFrame. "
            ...
            if not new_cols and new_col_count == 0:
                # No new columns were added - this might indicate the code didn't work
                # This could happen if:
                # 1. The LLM generated code that doesn't create features
                # 2. The code has errors that were silently ignored
                # 3. The code creates features but with existing column names
                import warnings
                warnings.warn(
                    f"No new columns were added after executing generated code. "
                    f"Original columns: {original_col_count}, "
                    f"After execution: {len(df_result.columns)}. "
                    f"This might indicate the generated code didn't create features. "
                    f"Try using mode='code' to review the generated code first.\n"
                    f"Generated code:\n{generated_code}",
                    UserWarning
                )
